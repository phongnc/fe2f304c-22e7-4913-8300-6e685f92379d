...
namespace AddTextToImage.WebUI.Controllers
{
    ...
    public class ImageController : ApiController
    {
        ...
        [HttpGet]
        public HttpResponseMessage Result2(int id)
        {
            HttpResponseMessage response;
            ...
            var modelItem = model.Items.First();
            string fullText = modelItem.Text;
            string[] listText = fullText.Split(new char[] { ',' });
            using (Ionic.Zip.ZipFile zip = new Ionic.Zip.ZipFile())
            {
                for (int i = 0; i < listText.Length; i++)
                {
                    ...
                    bmpResult.Save(itemStream, ImageFormat.Png);
                    zip.AddEntry(listText[i] + ".png", itemStream.ToArray());
                }
            ...
            string zipName = String.Format("tiles-{0}.zip", DateTime.Now.ToString("yyyy-MMM-dd-HHmmss"));
